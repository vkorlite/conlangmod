...
char **strsplit(char *input, char delim){
    ...
    for(i = 0; *(input+i) != '\0' && trc < len; i++)
        if(*(input+i) == delim){
            *(output+trc) =(char*) malloc((i-prev_space+1)*sizeof(char));
            for(int j = prev_space; j < i; j++)
                *(*(output+trc)+j-prev_space) = *(input+j);
            *(*(output+trc++)+i-prev_space) = '\0';
            prev_space = i+1;
        }
    *(output+trc) = (char*)malloc( (strlen(input)-prev_space+1) * sizeof(char));
...
char **strspits(char *input, char *delim){
    ...
    int act_len = 0;
    int prev_space = 0;
    char **output_raw =(char**) malloc(strlen(input) * sizeof(char*));
    for(i = 0; *(input+i) != '\0'; i++){
        mask &= (1 << del_len) - 1;
        mask <<= 3;
        mask += *(input+i);
        if(mask == del_mask){
            *(output_raw+act_len) =(char*) malloc((i-del_len-prev_space+1)*sizeof(char));
    ...
    char **output = (char**)malloc(act_len * sizeof(char*));
    for(i = 0; i < act_len; i++)
...
char **arena_strsplit(st_Arena *arena, char *input, char delim){
    ...
    for(i = 0; *(input+i) != '\0'; i++)
        if(*(input+i) == delim)
            len++;
    len++;
    char **output = arena_alloc(arena, len * sizeof(void*));
    int prev_space = 0;
    int trc = 0;
    for(i = 0; *(input+i) != '\0' && trc < len; i++)
        if(*(input+i) == delim){
            *(output+trc) = arena_alloc(arena, (i-prev_space+1)*sizeof(char));
            for(int j = prev_space; j < i; j++)
                *(*(output+trc)+j-prev_space) = *(input+j);
            *(*(output+trc++)+i-prev_space) = '\0';
            prev_space = i+1;
        }
    *(output+trc) = arena_alloc( arena, (strlen(input)-prev_space+1) * sizeof(char));
    for(i = prev_space; *(input+i) != '\0'; i++)
        *(*(output+trc)+i-prev_space) = *(input+i);
    *(*(output+trc)+i-prev_space) = '\0';
    return output;
